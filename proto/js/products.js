...
function updateProducts(new_products) {
    ...
    var template = ' <article class="product row" onclick="window.location.href=getProductURL({$product.idproduct});"> <span class="col-xs-12"> <h5 class="product-list-name">{$product.name}</h5> <span class="price-tag">&euro;{$product.price}</span> </span> <hr/> <div class="col-sm-3"><img src="http://placehold.it/350x250" class="img-responsive img-rounded product-image" /></div> <div class="col-sm-9 product-list-description"> <p><b>#{$product.code}</b></p> <p>{$product.description}</p> </div> </article>';
